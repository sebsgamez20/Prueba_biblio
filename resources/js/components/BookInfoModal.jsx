...
function BookInfoModal({ book, onClose, onRent, isRenting = false }) {
    ...
    `;
    const [showConfirmRent, setShowConfirmRent] = useState(false);
    ...
    const handleRentClick = () => {
        setShowConfirmRent(true);
    };

    const handleConfirmRent = () => {
        onRent(book.id);
        setShowConfirmRent(false);
    };
    ...
    return (
        <>
            ...
            {/* Modal de confirmación de renta */}
            {showConfirmRent && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-60 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full animate-in zoom-in-95 duration-200">
                        <div className="text-center">
                            <div className="w-16 h-16 bg-[#0000ab]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                                <svg className="w-8 h-8 text-[#0000ab]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <h3 className="text-lg font-semibold text-gray-800 mb-2">Confirmar Renta</h3>
                            <p className="text-gray-600 mb-6">
                                ¿Estás seguro de que quieres rentar <strong>"{book.title}"</strong>?
                            </p>
                            <div className="flex space-x-3">
                                ...
                                <button
                                    ...
                                    className="flex-1 px-4 py-2 bg-[#0000ab] text-white rounded-lg hover:bg-[#0000ab]/90 transition-colors duration-200"
                                >
                                    Confirmar
                                </button>
                            </div>
